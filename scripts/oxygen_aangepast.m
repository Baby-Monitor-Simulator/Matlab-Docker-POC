...
% Function for Pm from Cm
eq0 = @(Pm) Cm - alfa*Hbm ./ (1 + (23400 ./ (Pm.^3 + 150*Pm))) - beta*Pm;
Pm_candidate = fzero(eq0, [0, 140]); % adjust range as needed
if isreal(Pm_candidate) && Pm_candidate > 0
    PmO2(ico) = Pm_candidate;
else
    error('Geen positieve reële oplossing gevonden voor Pm');
end

% Equations necessary to solve initial values
eq1 = @(C1, P1, P2) (1-SFm)*mqutven*(Cm-C1) - D*(P1-P2);
eq2 = @(C2, Cf, P1, P2) (1-SFf)*mfqummc*(Cf-C2) + D*(P1-P2);
eq3 = @(C3, Cf)  mfqmc*(Cf-C3) - dVfdt0;
eq4 = @(C4, Cf) mfqbr*(Cf-C4) - dVfbrdt0;
eq5 = @(C2, C3, C4, Cf) Cf - ((C4*mfqbr +C3*mfqmc +(1-SFf)*C2*mfqummc)/(mfqbr+mfqmc+(1-SFf)*mfqummc));
eq6 = @(Pf, Cf) Cf - (alfa*Hbf/(1+(10400/(Pf^3+150*Pf)))+beta*Pf);
eq7 = @(P1, C1) C1 - (alfa*Hbm/(1+(23400/(P1^3+150*P1)))+beta*P1);
eq8 = @(P2, C2) C2 - (alfa*Hbf/(1+(10400/(P2^3+150*P2)))+beta*P2);
eq9 = @(P3, C3) C3 - (alfa*Hbf/(1+(10400/(P3^3+150*P3)))+beta*P3);
eq10= @(P4, C5 )C4 - (alfa*Hbf/(1+(10400/(P4^3+150*P4)))+beta*P4);
