...
function result = FMP_aanpassingen(ncyclemax, scen, HES, persen, server, should_stop)
...
while qrst <= ncyclemax
    % === FMP body parts options: 1 = present; 0 = not present
    mother    =1;
    uterus    =1;
    foetus    =1;
    umbilical =1; if umbilical==1; disp('umbilical=1'); elseif umbilical==2; disp('PAS OP: umbilical=2'); end
    brain     =1;
    CAVmodel  =2; %1=TVE, 2=1fiber 3=testcombi 5=1fibernew
    %scen      =0; % in funtie toegevoegd
    if scen==0; disp('normaal CTG'); elseif scen==1; disp('vroege decels'); elseif scen==2; disp('late decels'); elseif scen==3||scen==3.1; disp('variabele decels'); umbilical=2; end
    %HES       =0; %1 is fluid bolus (500 mL HES), 0 niet
    %persen    =0; %1 is meepersen, 0 niet
    duty      =0; %wel of geen duty cycles
    %ncyclemax =1000; in de functie toegevoegd        % [-] - maximum number of (maternal) cycles
            ...
            if j==round(sum(ftcycle(1:fcycle-1))/dt+kwart*ftcycle(fcycle)/(8*dt)) && j>=ftcycle(1)/dt ;
                ...
                if kwart==8
                    ...
                    elseif foetus==1 && convf==2 && convm==2
                        ...
                        oxygen_aangepast %syms weggehaald, want op een of andere manier deed syms het niet bij oxygen, maar wel bij pressure?
