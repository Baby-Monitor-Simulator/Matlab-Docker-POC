...
% === FMP body parts options: 1 = present; 0 = not present
mother    =1;
uterus    =1;
foetus    =1;
...
scen      =0; if scen==0; disp('normaal CTG'); elseif scen==1; disp('vroege decels'); elseif scen==2; disp('late decels'); elseif scen==3||scen==3.1; disp('variabele decels'); umbilical=2; end
HES       =0; %1 is fluid bolus (500 mL HES), 0 niet
persen    =0; %1 is meepersen, 0 niet
duty      =0; %wel of geen duty cycles
ncyclemax =1000;                  % [-] - maximum number of (maternal) cycles
...
m_cycle_time = 60/mHR; %zelf toegevoegd
%pressure_mother_l = zeros(1,nmax);
oxygen_mother_l = zeros(1,nmax);

disp(['initial is gedaan, J ' num2str(j)])
        ...
        if j==round(sum(ftcycle(1:fcycle-1))/dt+kwart*ftcycle(fcycle)/(8*dt)) && j>=ftcycle(1)/dt ;
            ...
            if kwart==8
                ...
                elseif convf==3 && mother==1;
                    disp('Calculation of oxygen values met chemoreceptor')
                    tom(ico)=j*dt;
                    tso=(tom(ico)-tom(ico-1))*1e-3;%(j0-j1)*dt*1e-3;
                    oxygen4;   % >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> user m-file
                    chemo_f5;
                    % disp(['Ico is ' num2str(ico)])
                    % disp(['J is ' num2str(j)])
                    disp(['Kijken hoe het loopt, J ' num2str(j) ',ico ' num2str(ico) ' , Icycle ' num2str(icycle) ',tom(ico) = ' num2str(tom(ico))])
                    oxygen_mother_l(1,ico-1) =  (j*dt)/1000 -60;
                end
            end
...
% %                    
    ...
    %pressure_mother = struct('LV_pressure', mpsav(1,j)*fackPaHg);%, 'Time', t);%, ...
    %'SA_pressure', mpsav(2,j)*fackPaHg);
    % pressure_mother_l(1,j) = mpsav(1,j)*fackPaHg;
    % pressure_mother_l(2,j) = mpsav(1,j)*fackPaHg;
    % pressure_mother_l(3,j) = mqsav(3,j)*60*1000;
    % pressure_mother_l(4,fcycle) = 60./(ftcycle(fcycle)/1000);
    %oxygen_mother_l(1,j) =  tom(ico);
...
if foetus==1
    ...
    fq = fqsav(:,jmin:j);
    post_f % >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> user m-file
    %pressure_mother_l(4,j) =  PmO2(ico-1);
end